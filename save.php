...
if ($link == '') {
    ...
    $abp = mysqli_query("SELECT * FROM `short` WHERE `little` = '$what' ", $db);    // проверка на повторение
    $apb = mysqli_fetch_row($abp);                                                    // костыль или нет?
    ...
    if ($apb == '') {
        if ($auto == true) {
            ...
            $id = 0;
        } // Если 6 - релинка не будет, если 0 - будет
        if ($nolink == true) {
            $id = 12;
        } // Если 12 - покажу без ссылки. Костыль, да.
